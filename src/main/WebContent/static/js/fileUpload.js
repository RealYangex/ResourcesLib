...
var uploadTools = {
    ...
    "initWithLayout":function(opt){
        ...
        var proceess = '<div class="progress" id="progress" style="display:none">'
        +'<div class="progress-bar progress-bar-striped active" role="progressbar" aria-valuenow="100" aria-valuemin="0" aria-valuemax="100" style="width: 100%">'
        +'<span class="sr-only">45% Complete</span></div></div>';
        $("#"+uploadId).append(proceess);
        //添加文件显示框
        ...
        if(uploadUrl!="#"&&uploadUrl!=""){
            ...
            $.ajax({
                ...
                success:function(data){
                	setTimeout(function(){$("#progress").css("display","none");},1000);
                	uploadTools.initWithCleanFile(opt);
                    setTimeout(function(){opt.onUpload(opt,data)},500);
                    if(opt.isAutoClean){
                        setTimeout(function () {uploadEvent.cleanFileEvent(opt);},1000) ;
                    }
                },
                error:function(e){
